function OnUpdate(doc,meta) {
    try {
        ...
        if (!all_jobs_document.hasOwnProperty('cblite')) {
            all_jobs_document['cblite'] = {};
        }
        if (!all_jobs_document['cblite'].hasOwnProperty(os)) {
            all_jobs_document['cblite'][os] = {};
        }
        if (!all_jobs_document['cblite'][os].hasOwnProperty(component)) {
            all_jobs_document['cblite'][os][component] = {};
        }
        //const implementedIn = get_implemented_in(component, sub_component);
        const version = build_version.split('-')[0];
        if (!(name in all_jobs_document['cblite'][os][component])) {
            all_jobs_document['cblite'][os][component][name] = {
            ...
            if (all_jobs_document['cblite'][os][component][name]['jobs_in'].indexOf(version) == -1){
                all_jobs_document['cblite'][os][component][name]['jobs_in'].push(version);

                all_jobs_document['cblite'][os][component][name]['jobs_in'] =
                    all_jobs_document['cblite'][os][component][name]['jobs_in'].sort().filter(function(item, pos, ary){
        ...
        if (doc.hasOwnProperty('build_type')){
            build_to_store['build_type'] = doc['build_type']
        }
        if (doc.hasOwnProperty('sync_gateway_version')){
            build_to_store['sync_gateway_version'] = doc['sync_gateway_version']
        }
        ...
        const doc_id = build_version.concat("_cblite")
        tgt[doc_id] = doc_to_insert;

        if (update_all_jobs_document) {
            tgt['existing_builds_cblite'] = all_jobs_document
            for(let i=0; i < 1; i++){
              let valid = validateExistingBuilds(doc,all_jobs_document["cblite"][os][component][name]);
              if (valid) {
                  break;
              }
            }
...
function get_all_jobs_document(){
    ...
    try {
        all_jobs_document = tgt['existing_builds_cblite'];
    } catch (e) {
        all_jobs_document = {};
    }
...
function validateExistingBuilds(doc,build_to_store){
    try{
      ...
      function upsertDoc(doc_to_upsert){
        tgt["existing_builds_cblite"] = doc_to_upsert;
      }


      if(existing_build_doc["cblite"].hasOwnProperty(os)){
        if(existing_build_doc["cblite"][os].hasOwnProperty(component)){
          if(existing_build_doc["cblite"][os][component].hasOwnProperty(name)){
            let existing_job = existing_build_doc["cblite"][os][component][name]
            if(existing_job["totalCount"] == build_to_store["totalCount"] &&
              ...
              valid_data = false
              existing_build_doc["cblite"][os][component][name] = build_to_store
          ...
          existing_build_doc["cblite"][os][component] = {}
          existing_build_doc["cblite"][os][component][name] = build_to_store
          upsertDoc(existing_build_doc)
        }
      }
      else{
        existing_build_doc["cblite"][os] = {}
        existing_build_doc["cblite"][os][component] = {}
        existing_build_doc["cblite"][os][component][name] = build_to_store
        upsertDoc(existing_build_doc)
      }
