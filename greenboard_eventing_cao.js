function OnUpdate(doc,meta) {
    try {
        ...
        if (!all_jobs_document.hasOwnProperty('operator')) {
            all_jobs_document['operator'] = {};
        }
        if (!all_jobs_document['operator'].hasOwnProperty(os)) {
            all_jobs_document['operator'][os] = {};
        }
        if (!all_jobs_document['operator'][os].hasOwnProperty(component)) {
            all_jobs_document['operator'][os][component] = {};
        }
        //const implementedIn = get_implemented_in(component, sub_component);
        const version = build_version.split('-')[0];
        if (!(name in all_jobs_document['operator'][os][component])) {
            all_jobs_document['operator'][os][component][name] = {
            ...
            if(doc.hasOwnProperty('server_version')) {
                all_jobs_document['operator'][os][component][name]['server_version'] = doc['server_version'];
            }
            update_all_jobs_document = true;
        }
        else {
            if (all_jobs_document['operator'][os][component][name]['jobs_in'].indexOf(version) == -1){
                all_jobs_document['operator'][os][component][name]['jobs_in'].push(version);

                all_jobs_document['operator'][os][component][name]['jobs_in'] =
                    all_jobs_document['operator'][os][component][name]['jobs_in'].sort().filter(function(item, pos, ary){
        ...
        const doc_id = build_version.concat("_operator")
        tgt[doc_id] = doc_to_insert;

        if (update_all_jobs_document) {
            tgt['existing_builds_operator'] = all_jobs_document
            for(let i=0; i < 1; i++){
              let valid = validateExistingBuilds(doc,all_jobs_document["operator"][os][component][name]);
              if (valid) {
                  break;
              }
            }
...
function get_all_jobs_document(){
    ...
    try {
        all_jobs_document = tgt['existing_builds_operator'];
    } catch (e) {
        all_jobs_document = {};
    }
...
function validateExistingBuilds(doc,build_to_store){
    try{
      ...
      function upsertDoc(doc_to_upsert){
        tgt["existing_builds_operator"] = doc_to_upsert;
      }


      if(existing_build_doc["operator"].hasOwnProperty(os)){
        if(existing_build_doc["operator"][os].hasOwnProperty(component)){
          if(existing_build_doc["operator"][os][component].hasOwnProperty(name)){
            let existing_job = existing_build_doc["operator"][os][component][name]
            if(existing_job["totalCount"] == build_to_store["totalCount"] &&
              ...
              valid_data = false
              existing_build_doc["operator"][os][component][name] = build_to_store
          ...
          existing_build_doc["operator"][os][component] = {}
          existing_build_doc["operator"][os][component][name] = build_to_store
          upsertDoc(existing_build_doc)
        }
      }
      else{
        existing_build_doc["operator"][os] = {}
        existing_build_doc["operator"][os][component] = {}
        existing_build_doc["operator"][os][component][name] = build_to_store
        upsertDoc(existing_build_doc)
      }
