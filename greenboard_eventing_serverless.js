function OnUpdate(doc, meta) {
    try {
        ...
        if (!all_jobs_document.hasOwnProperty('serverless')) {
            all_jobs_document['serverless'] = {};
        }
        if (!all_jobs_document['serverless'].hasOwnProperty(os)) {
            all_jobs_document['serverless'][os] = {};
        }
        if (!all_jobs_document['serverless'][os].hasOwnProperty(component)) {
            all_jobs_document['serverless'][os][component] = {};
        }
        //const implementedIn = get_implemented_in(component, sub_component);
        const version = build_version.split('-')[0];
        if (!(name in all_jobs_document['serverless'][os][component])) {
            all_jobs_document['serverless'][os][component][name] = {
            ...
            if (all_jobs_document['serverless'][os][component][name]['jobs_in'].indexOf(version) == -1) {
                all_jobs_document['serverless'][os][component][name]['jobs_in'].push(version);

                all_jobs_document['serverless'][os][component][name]['jobs_in'] =
                    all_jobs_document['serverless'][os][component][name]['jobs_in'].sort()
...
function get_all_jobs_document() {
    ...
    try {
        all_jobs_document = tgt['existing_builds_serverless'];
    } catch (e) {
        all_jobs_document = {};
    }
...
function validateExistingBuilds(doc, build_to_store) {
    try {
        ...
        function upsertDoc(doc_to_upsert) {
            tgt["existing_builds_serverless"] = doc_to_upsert;
        }

        if (existing_build_doc["serverless"].hasOwnProperty(os)) {
            if (existing_build_doc["serverless"][os].hasOwnProperty(component)) {
                if (existing_build_doc["serverless"][os][component].hasOwnProperty(name)) {
                    let existing_job = existing_build_doc["serverless"][os][component][name]
                    if (existing_job["totalCount"] == build_to_store["totalCount"] &&
                        ...
                        valid_data = false;
                        existing_build_doc["serverless"][os][component][name] = build_to_store;
                ...
                existing_build_doc["serverless"][os][component] = {};
                existing_build_doc["serverless"][os][component][name] = build_to_store;
                upsertDoc(existing_build_doc);
            }
        } else {
            existing_build_doc["serverless"][os] = {};
            existing_build_doc["serverless"][os][component] = {};
            existing_build_doc["serverless"][os][component][name] = build_to_store;
            upsertDoc(existing_build_doc);
        }
                    ...
                    if (doc_to_insert['os'][os][component].hasOwnProperty(name)) {
                        ...
                        if (job_to_check !== undefined) {
                            if (job_to_check['totalCount'] === doc['totalCount'] &&
                                ...
                                doc_to_insert['os'][os][component][name][index_of_job] = 
                                    new_build_to_store;
